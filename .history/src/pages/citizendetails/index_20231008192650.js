...
import { getOneCD } from '../../services/qlcd_services';
...
function Citizendetails() {
    ...
    const getCD = async () => {
        try {
            const response = await getCDWithCCCD(cccd);
            setCitizen(response.CongDan);
            setAdressTamT(response.DiaChiTamTru);
            setAdressTTru(response.DiaChiThuongTru);
        } catch (error) {
            console.log('lỗi');
        }
        console.log(citizen, adressTamT, adressTTru);
    };
    return (
