...
function AddCitizen() {
    ...
    const handleNgaySinhChange = (date, dateString) => {
        setNgaySinh(dateString); // dateString chứa ngày đã chọn dưới dạng chuỗi.
    };
    ...
    const handleDanTocChange = (e) => {
        const value = e.target.value;
        setDanToc(value);
    };
    
    const handleQuocTichChange = (e) => {
        const value = e.target.value;
        setQuocTich(value);
    };
    ...
    const handleAddAddress = () => {
        ...
        axios.post('http://localhost:8000/api/themCongDan', newCD)
            .then((response) => {
