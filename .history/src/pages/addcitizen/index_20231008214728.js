...
function AddCitizen() {
    ...
    async function createAndAssignDCID(dataCD, DCData) {
        try {
          // Thêm bản ghi mới cho địa chỉ thường trú vào collection DC
          const DCInsertResult = await db.collection('DC').insertOne(DCData);
      
          // Lấy _id của địa chỉ thường trú sau khi thêm vào collection
          const DCID = DCInsertResult.insertedId;
      
          // Gán _id của địa chỉ thường trú cho DiaChiThuongTru trong thông tin cá nhân của CD
          dataCD.DiaChiThuongTru = DCID;
      
          // Thêm thông tin cá nhân của CD vào collection CD
          await db.collection('CD').insertOne(dataCD);
      
          console.log('Tạo thông tin cá nhân của CD và gán DiaChiThuongTru thành công!');
        } catch (error) {
          console.error('Lỗi khi tạo thông tin cá nhân của CD: ', error);
        }
      }
