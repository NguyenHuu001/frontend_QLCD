...
import { searchCCCD, getOneCD, getOneTT, editDataCitizen } from '../../services/qlcd_services';
...
function EditCitizen() {
    const cancuoc = localStorage.getItem('CCCD');
    const idCD = localStorage.getItem('idCD');
    const idDC = localStorage.getItem('idDC');
    const idTT = localStorage.getItem('idTT');
    const [IDCC, setIDCC] = useState(cancuoc);
    const [IDCD, setIDCD] = useState(idCD);
    const [IDDC, setIDDC] = useState(idDC);
    const [IDTT, setIDTT] = useState(idTT);
    // const [citizen, setCitizen] = useState();
    // const [citizenDC, setCitizenDC] = useState();
    // const [citizenTT, setCitizenTT] = useState();
    ...
    const fetchDetailCitizen = async () => {
        try {
            const responseCD = await searchCCCD(IDCC).then((res) => {
                res.map((item) => {
                    setSoCCCD(item.SoCCCD);
                    setHoTen(item.HoTen);
                    setNgaySinh(item.NgaySinh);
                    setGioiTinh(item.GioiTinh);
                    setDanToc(item.DanToc);
                    setQuocTich(item.QuocTich);
                    setQueQuan(item.QueQuan);
                });
            });

            const responseDC = await getOneCD(IDDC).then((res) => {
                setHouseNumber(res.SoNha);
                setStreetName(res.TenDuong);
                setSelectedCity(res.TinhThanh);
                setSelectedDistrict(res.QuanHuyen);
                setSelectedWard(res.PhuongXa);
            });

            const responseTT = await getOneTT(IDTT).then((res) => {
                setsoCCCDTT(res.SoCCCD);
                setHoTenTT(res.HoTen);
                setSoDTTT(res.SoDT);
                setEmailTT(res.Email);
                setHouseNumberTT(res.DiaChi.SoNha);
                setStreetNameTT(res.DiaChi.TenDuong);
                setSelectedCityTT(res.DiaChi.TinhThanh);
                setSelectedDistrictTT(res.DiaChi.QuanHuyen);
                setSelectedWardTT(res.DiaChi.PhuongXa);
            });
        } catch (error) {
            console.log('lỗi');
        }
    ...
    const EditDetailCitizen = async () => {
        ...
        const dataCD = {
            ...
            selectedWardTT: selectedWardTT || '',
            selectedDistrictTT: selectedDistrictTT || '',
            selectedCityTT: selectedCityTT || '',
        };
        try {
            const response = await editDataCitizen(dataCD);
            setTimeout(() => {
        ...
        <div className="container mt-4 p-0">
            <div className="d-flex flex-wrap">
                ...
                <div className="col-md-6 d-flex flex-column justify-content-between ps-2">
                    ...
                    <button onClick={openImagePicker}>Chọn ảnh</button>
                    <div className="d-flex justify-content-between">
                        <Input
                            placeholder="Số nhà"
                            value={houseNumber}
                ...
                <div className="d-flex align-items-center mt-4">
                    <input type="checkbox" style={{ height: '100%' }} onClick={Hide} />
                    <b className="ms-2">Sửa tạm trú</b>
                </div>
                {hide && (
                    <div className="col-md-12 d-flex justify-content-center mt-5" style={{ minHeight: '280px' }}>
                        <div className="col-md-8 d-flex flex-column justify-content-between align-items-between ps-2">
                            <div className="d-flex justify-content-center">
                                <h3 style={{ textTransform: 'uppercase', color: '#a04244' }}>
                                    Thông tin Địa chỉ tạm trú
                                </h3>
                            </div>
                ...
                <div className="col-md-12 d-flex justify-content-center mt-5">
                    <div className="btn_Address_add" onClick={() => EditDetailCitizen()}>
                        Sửa thông tin
                    </div>
                </div>
