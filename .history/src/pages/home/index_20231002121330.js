...
function Home() {
    ...
    const [dctt, setDiaChiThuongTru] = useState({});
    useEffect(() => {
        getAllCongDan();
    }, []);
    ...
    const getOneCongDan = async (id) => {
        try {
            console.log(id);
            const response = await getOneCD(id);
            console.log(response);
            setDiaChiThuongTru(response); // Lưu thông tin địa chỉ thường trú vào state
        } catch (error) {
            console.log('Lỗi');
        }
    };
                ...
                congDan.map((item, index) => (
                    <div className="d-flex mt-3 no-wrap align-items-center" key={item._id}>
                        ...
                        <div className="col-md-3 item">
                            <button className="btn_detail"  onClick={() => getOneCongDan(item.DiaChiThuongTru)}>Hiển thị</button>
                        </div>
