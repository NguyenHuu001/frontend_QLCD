...
function Home() {
    ...
    const getDiaChi = (id) => {
        ...
        if (!matchedItem) return 'Chưa Có Địa Chỉ'; // Đảm bảo rằng matchedItem tồn tại
        if (!matchedItem.SoNha) return 'Chưa Có Địa Chỉ';
    ...
    const deleteCitizen = async (_IDCD, _IDDC, _IDTT) => {
        try {
            await deleteCD(_IDCD);
            await deleteDC(_IDDC);
            await deleteTT(_IDTT);
            console.log('xóa thanhfg cong');
            getDiaChiAll();
            getAllCongDan();
        } catch (error) {
            console.log('LỖi');
                ...
                congDan.map((item, index) => (
                    <div key={item._id}>
                        <div className="d-flex mt-3 no-wrap align-items-center">
                            <div className="col-md-1 item" style={{height:'20px'}}>{index + 1}</div>
                            <div className="col-md-2 item" style={{height:'20px'}}>{item.HoTen}</div>
                            <div className="col-md-1 item" style={{height:'20px'}}>{item.NgaySinh}</div>
                            <div className="col-md-1 item" style={{height:'20px'}}>{item.GioiTinh}</div>
                            <div className="col-md-1 item" style={{height:'20px'}}>{item.SoCCCD}</div>
                            <div className="col-md-3 item" style={{height:'20px'}}>{getDiaChi(item.DiaChiThuongTru)}</div>
                            ...
                            <button
                                ...
                                onClick={() => deleteCitizen(item._id, item.DiaChiThuongTru, item.DiaChiTamTru)}
                            >
                                Xóa
                            </button>
                        </div>
