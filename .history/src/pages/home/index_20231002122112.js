...
function Home() {
    ...
    const getOneCongDan = async (id) => {
        try {
            ...
            setdctt(response); // Lưu thông tin địa chỉ thường trú vào state
        } catch (error) {
            console.log('Lỗi');
        }
    };
                ...
                congDan.map((item, index) => (
                    <div className="d-flex mt-3 no-wrap align-items-center" key={item._id}>
                        ...
                        <div className="col-md-3 item">
                            {hide ? (
                                <button className="btn_detail" onClick={() => getOneCongDan(item.DiaChiThuongTru)}>
                                    Hiển thị
                                </button>
                            ) : (<div className="item">{dctt.PhuongXa} </div>)}
                        </div>
